...
- Explicit cold-start handling (< 3 interactions → skip collab)
...
class PopularityRecommender:
    ...
    def fit(self, events_df: pd.DataFrame):
        ...
        Takes ~1 second on 2.75M events.
        """
    ...
    def recommend(self, n: int = 10, exclude_items: set = None) -> list:
        """
        Return top-N popular items, excluding already-seen items.
        O(n) time — extremely fast.
        """
...
class CoOccurrenceRecommender:
    ...
    - Memory: O(U × k²) where k = avg items per user ≈ 2-3
    ...
    MIN_INTERACTIONS = 3  # minimum to activate this layer
    ...
    def fit(self, events_df: pd.DataFrame):
        ...
        Only strong signals — views are too noisy.
        """
        # Strong signals only
        strong = events_df[
        ...
        # Build user → items mapping (vectorized, no iterrows)
        user_item_groups = (
        ...
        for items in user_item_groups.values():
            unique_items = list(set(items))  # deduplicate per user
        ...
        # Apply IDF dampening
        # Score(A→B) = co_occur(A,B) / log(1 + popularity(B))
        # This prevents popular items from dominating every recommendation
    ...
    def recommend(self, visitorid: int, n: int = 10,
        ...
        """
        Recommend items based on what co-occurs with user's history.
        Returns empty list if user has < MIN_INTERACTIONS (cold start).
        """
        if not self.fitted:
            return []
        ...
        # Cold start: not enough signal, skip this layer
        ...
        # Rank by total co-occurrence score
        ranked = sorted(
...
class SegmentAwareRecommender:
    ...
    - Power Buyer:      high-value, high-frequency → trending premium items
    - Buyer:            occasional buyer → complementary items  
    - Engaged Browser:  cart but no buy → items with high cart-to-buy ratio
    - Window Shopper:   views only → most viewed, aspirational items
    - Passive Visitor:  minimal activity → top popular items
    ...
    def fit(self, events_df: pd.DataFrame, twins_df: pd.DataFrame):
        ...
        # Merge segment info onto events
        segment_map = twins_df.set_index('visitorid')['customer_segment'].to_dict()
...
class HybridRecommender:
    ...
    HIGH_INTENT   (buy_prob >= 0.5, interactions >= 3)
    → 60% collaborative + 25% segment + 15% popularity
    → Trust the model: this person knows what they want
    
    MEDIUM_INTENT (buy_prob >= 0.2 OR interactions >= 3)  
    → 35% collaborative + 40% segment + 25% popularity
    → Balance personalization with discovery
    
    COLD_START    (new user, < 3 interactions)
    → 0% collaborative + 50% segment + 50% popularity
    → No signal yet: use group behavior + popular items
    
    DISCOVERY     (low probability, minimal interactions)
    → 10% collaborative + 40% segment + 50% popularity
    → Show them what's popular in their group
    """
    ...
    def fit(self, events_df: pd.DataFrame, twins_df: pd.DataFrame):
        ...
        # Index twins by visitorid for O(1) lookup
        ...
        self.cooccurrence.fit(events_df)

        print("Layer 3: Segment-Aware Recommender")
        self.segment_aware.fit(events_df, twins_df)
    ...
    def _get_strategy(self, buy_prob: float,
        ...
        Returns (collab_weight, segment_weight, popular_weight, strategy_name)
        """
    ...
    def recommend(self, visitorid: int, n: int = 10) -> dict:
        ...
        Returns dict with recommendations + metadata for explainability.
        """
        ...
        # --- Popularity fill (always fills remaining slots) ---
